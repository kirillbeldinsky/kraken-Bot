#!/usr/bin/env python3
# kraken_bot_v6.2_full.py
# Version: 6.2.0
# Changes:
# 1. Fixed TP - frozen at entry, doesn't recalc on each tick
# 2. Hourly [STATS] output: balance, trades, winrate, pnl, daily_r
# 3. Version string in log header
# 4. Volatility filter 1.5% + dynamic TP 0.7%-1.5%
# 5. Daily loss limit 2R
# 6. Trailing stop after 0.5% move
...
STATS_INTERVAL_TICKS = 4  # 15min * 4 = 1 hour
...
def load_state():
    defaults = {
        ...
        "tick_counter": 0, "total_trades": 0, "total_wins": 0, "total_pnl": 0.0
    }
    try:
    ...
    for k, v in defaults.items():
        state.setdefault(k, v)   # ← вот миграция
    return state
...
def get_ohlc():
    ...
    ohlc = data.get("XXBTZUSD") or data.get("XBTUSD")
...
def log_stats():
    ...
    logging.info(f"[STATS] Баланс: ${stats['paper_balance']:.2f} | Сделок: {total} | Винрейт: {winrate:.0f}% | PNL: ${stats['total_pnl']:.2f} | Сегодня: {stats['daily_loss_r']:.2f}R")

stats = load_state()
logging.info(f"=== Старт бота v{VERSION}: TP фиксируется при входе + стата каждый час ===")
...
while True:
    try:
        ...
        if stats["tick_counter"] >= STATS_INTERVAL_TICKS:
            log_stats()
