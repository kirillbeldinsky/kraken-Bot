#!/usr/bin/env python3
# kraken_bot_v6.4.2.py
# Version: 6.4.2
# Changes:
# 1. Логи пропусков раз в INTERVAL, не каждую минуту
# 2. Telegram уведомления: [STATS] и [CLOSE]
# 3. Запись equity.csv: timestamp,balance,pnl,total_trades
# 4. Фильтр объёма, спреда, cooldown, paper slippage, fee 0.16%
...
def send_telegram(text):
	...
	try:
		requests.post(url, data={"chat_id": TELEGRAM_CHAT_ID, "text": text, "parse_mode": "Markdown"}, timeout=5)
	except Exception as e:
		logging.error(f"Telegram send failed: {e}")

def save_equity(balance, pnl):
	header_needed = not os.path.exists(EQUITY_FILE)
	with open(EQUITY_FILE, "a") as f:
		if header_needed: f.write("timestamp,balance,pnl,total_trades\n")
		f.write(f"{datetime.now().isoformat()},{balance:.2f},{pnl:.2f},{stats['total_trades']}\n")
...
def kraken_request(uri_path, data):
	if not API_KEY or not API_SECRET: return {"error": ["EAPI: no keys"]}
...
def get_ohlc():
	...
	ohlc = resp.json()["result"].get("XETHZUSD") or resp.json()["result"].get("ETHUSD")
...
def get_ticker():
	...
	d = t.get("XETHZUSD") or t.get("ETHUSD")
...
def check_exit(mp):
	...
	if slh or tph:
		...
		msg=f"[CLOSE] {'STOP' if slh else 'TP'} {s.upper()} @{ex:.2f} PnL${pnl:.2f} fee${fees:.2f}"
		logging.info(msg)
		send_telegram(f"*{PAIR} [CLOSE]*\n{'🛑 STOP' if slh else '✅ TP'} {s.upper()} `@{ex:.2f}`\nPnL: `${pnl:.2f}` | Fee: `${fees:.2f}`\nBalance: `${stats['paper_balance']:.2f}`")
		save_equity(stats["paper_balance"], pnl)
...
def log_stats():
	...
	msg=f"[STATS] ${stats['paper_balance']:.2f} | {t} trades | {wr:.0f}% | PnL${stats['total_pnl']:.2f} | {stats['daily_loss_r']:.2f}R"
	logging.info(msg)
	send_telegram(f"*{PAIR} v{VERSION}*\nBalance: `${stats['paper_balance']:.2f}`\nTrades: `{t}` | WR: `{wr:.0f}%`\nPnL: `${stats['total_pnl']:.2f}`\nRegime: `{stats['current_regime']}` | DailyR: `{stats['daily_loss_r']:.2f}`")

stats=load_state()
logging.info(f"=== Старт v{VERSION} ===")
send_telegram(f"🚀 *Bot started* `{VERSION}`\nMode: `{TRADE_MODE}` | Pair: `{PAIR}`")
import threading
    ...
    while True:
        try:
            ...
            for update in resp.get("result", []):
                ...
                if str(chat_id) != str(TELEGRAM_CHAT_ID): continue  # только от тебя

                if text == "/start":
                    send_telegram("✅ Бот активен. Команды:\n`/stats` - баланс и PnL\n`/pos` - текущая позиция\n`/stop` - остановить бота")
                elif text == "/stats":
                    t=stats["total_trades"]; w=stats["total_wins"]; wr=(w/t*100) if t else 0
                    ...
                    if pos:
                        send_telegram(f"*📌 Position*\n{pos['side'].upper()} `{pos['volume']}` @ `${pos['entry']}`\nSL: `${pos['sl']}` | TP: `${pos['tp']}`")
                    else:
                        send_telegram("`Нет открытой позиции`")
                elif text == "/stop":
                    send_telegram("🛑 Останавливаю бота...")
                    os._exit(0)
        except Exception as e:
