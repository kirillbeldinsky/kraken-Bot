...
VERSION = "6.4.0-fee-volume-cooldown"
load_dotenv()
...
while True:
    try:
        ...
        if len(prices)<SMA_PERIOD: time.sleep(60); continue
        if ticker["spread_pct"]>SPREAD_MAX_PCT: logging.info("SKIP spread"); time.sleep(60); continue
        if len(volumes)>=20:
            av=sum(volumes[-20:])/20; rv=sum(volumes[-3:])/3
            if rv<av*VOLUME_THRESHOLD: logging.info(f"SKIP vol {rv:.1f}<{av*VOLUME_THRESHOLD:.1f}"); time.sleep(60); continue
        if stats.get("last_loss_time"):
            ll=datetime.fromisoformat(stats["last_loss_time"])
            if datetime.now()-ll<timedelta(minutes=INTERVAL*COOLDOWN_BARS): logging.info("COOLDOWN"); time.sleep(60); continue
